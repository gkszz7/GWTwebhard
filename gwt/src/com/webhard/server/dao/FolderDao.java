...
public class FolderDao implements Serializable{
	...
	public FolderDto selectHomeFolder() {
        ...
        try {
			con = connection.conn();
			String sql="select f.foldertype, f.step, i.itemNum, i.name, i.ITEM_CREATION_DATE, i.parentNum, i.userid, i.companyNum, i.type "
					+ "from folder f, item i where i.itemNum = f.itemNum and i.itemNum = 296";
			ps = con.prepareStatement(sql);
			rs=ps.executeQuery();
	...
	public ItemDto printCompFolderbyCompanyNum(int comNum) {
        ...
        try {
			con = connection.conn();
			String sql="select * from item where companyNum= ? and parentNum = 296";
			ps = con.prepareStatement(sql);
			ps.setInt(1, comNum);
			rs=ps.executeQuery();
	...
	public FolderDto printShareFolder() {
        ...
        try {
			con = connection.conn();
			String sql="select f.foldertype, f.step, i.itemNum, i.name, i.ITEM_CREATION_DATE, i.parentNum, i.userid, i.companyNum, i.type"
					+ " from folder f, item i where i.parentNum= 296 and f.foldertype = 1 and i.itemNum=f.itemNum";
			ps = con.prepareStatement(sql);
			rs=ps.executeQuery();
